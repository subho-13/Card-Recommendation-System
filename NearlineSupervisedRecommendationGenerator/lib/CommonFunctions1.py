...
def new_user_remover(df):
    num_users = len(df)
    cat_start = df.columns.get_loc('Education')
    sum_exp = df.iloc[:, cat_start:].sum(axis = 1).values
    cutoff = np.quantile(sum_exp, 0.1)
    old_ind = 0
    for i in range(len(df)-1, -1, -1):
        sum_user_exp = df.iloc[i, cat_start:].sum()
        if sum_user_exp > cutoff:
            old_ind = i
            break

    df_1 = df.iloc[:old_ind+1, :]
    empty_list = [[0]*9]*(num_users - old_ind - 1)
    return df_1, empty_list
...
def get_complimentary_card(complimentary_card_dict, df, user_id):
    ...
    comp_card = complimentary_recommendation_generator(user_arr, complimentary_card_dict, card_type_ind, cred_score_ind, job_ind)
    return comp_card

def card_generator(card_type_dict, card_dict, card_mapper, df):
    '''Generates Recommendations - 3 for KPCA Algorithm with K-Means.'''
    
    rec = []
    complimentary_card_dict = complimentary_card_generator()
    for i in range(0,len(card_dict)):
        if card_type_dict[i] != card_mapper[list(card_dict[i])[0]]:
            ...
            rec.append([i, get_complimentary_card(complimentary_card_dict, df, i), 2])
        else:
...
#     recommendation_3 = np.array(recommendation_3)
    ...
    df_extra1 = df_extra[df_extra['Rec_Type'] == 1].copy()
    ind = np.arange(0, len(df_extra1), 1)
    df_extra1 = df_extra1.set_index(ind)
    plt.figure(figsize = (12, 8))
    sns.countplot(x = 'Card_Name', data = df_extra1)
