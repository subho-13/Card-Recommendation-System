...
def user_cat_list_generator(user):
    ...
    cat_list = ['Education', 'Entertainment', 'Food', 'Gas_trans', 'Grocery_net', 'Grocery_pos',
                'Health', 'Home', 'Hotel', 'Kids_pets', 'Misc_net', 'Misc_pos', 'Personal', 'Shop_net', 'Shop_pos',
                'Travel']
...
def find_suggestions(user_association_list, df_prod, reward_points, factor=1.5):
    ...
    df_user = df_prod.loc[(df_prod['product_category'].isin(user_association_list)) & (
            df_prod['reward_points_required'] <= reward_points)]
